...
median_bandwidth <- function(x) {
  ...
  md <- sqrt(median(D[upper.tri(D, diag = FALSE)]))
...
cv_krr_fast <- function(x, y, weights = NULL, k_folds = 3,
  ...
  for (b in b_range) {
    ...
    for (f in seq_len(k_folds)) {
      ...
      ytr <- y[tr]
      if (is.null(weights)) W2 <- rep(1, sum(tr)) else W2 <- sqrt(pmax(as.numeric(weights[tr]), 0))
...
weight_kern <- function(object = NULL, x = NULL, w = NULL, y = NULL,
  ...
  if (is.null(x) || is.null(w) || is.null(y)) stop("Provide either `object` with @x,@y,@z, or raw `x, w, y`.")
  x <- as.matrix(x); w <- as.numeric(w); y <- as.numeric(y)
  n <- nrow(x); if (is.null(k_folds)) k_folds <- max(3, min(10, floor(n/4)))
  ...
  wt_tau <- zr^2
  ytilde_u <- (y - m_hat) / zr
  tau_u <- cv_krr_fast(x, ytilde_u, weights = wt_tau, k_folds = k_folds, b_range = b_range, lambda_range = lambda_range)
  tau0  <- sum((y - m_hat) * zr) / sum(wt_tau)
  beta1 <- coef(lm(y ~ w + p_hat))[2]
  ...
  for (lam in lambdas_blend) {
    ...
    for (kk in 1:2) {
      ...
      xi_fit <- cv_krr_fast(x[tr,, drop=FALSE], ystar_tr, k_folds = 2, b_range = b_range, lambda_range = lambda_range)
  ...
  tau_r <- cv_krr_fast(x, ytilde_r, weights = wt_tau, k_folds = k_folds, b_range = b_range, lambda_range = lambda_range)
  structure(list(
