...
weight_dnn <- function(object, k_folds = 5, en_dnn_ctrl = NULL) {
  ...
  for (k in 1:K) {
    ...
    ys0_mod_k <- do.call(deepTL::ensemble_dnnet, c(list(object = ys0_obj), en_dnn_ctrl))
    ys0_hat[te] <- as.numeric(deepTL::predict(ys0_mod_k, X[te, , drop = FALSE]))
  }
  
  ys0_obj_full <- deepTL::importDnnet(x = X, y = Ystar)
  ys0_mod_full <- do.call(deepTL::ensemble_dnnet, c(list(object = ys0_obj_full), en_dnn_ctrl))
  ...
  mod <- list(
    ...
    revised = list(tau_mod = tau_mod_r, ys0_mod = ys0_mod_full, tau0 = tau0)
  )
...
#' @param newx Matrix or data frame of new covariates.
...
#' @export
predict.weight_dnn <- function(object, newx,
                               which = c("both", "unrevised", "revised"), ...) {
  which <- match.arg(which)
  
  if (missing(newx) || is.null(newx)) {
    stop("Please provide `newx` (matrix of covariates) for prediction with weight_dnn objects.")
  }
