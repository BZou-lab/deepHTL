...
weight_dnn <- function(object, en_dnn_ctrl = NULL) {
  ...
  z_obj <- importDnnet(x = object@x, y = z_fac)
  z_mod <- do.call("ensemble_dnnet", c(list(object = z_obj), en_dnn_ctrl)) 
  pz <- predict(z_mod, object@x) 
  ...
  mu_hat <- as.numeric(predict(y_mod, object@x))
  
  w <- (z_num - e_hat)^2
  
  semi_dat <- importDnnet(
    x = object@x,
    y = (object@y - mu_hat) / (z_num - e_hat),
    w = w
  )
  
  tau_mod_u <- do.call("ensemble_dnnet", c(list(object = semi_dat), en_dnn_ctrl))
  
  tau0 <- sum((object@y - mu_hat) * (z_num - e_hat)) / sum((z_num - e_hat)^2)
  
  ys0_obj <- importDnnet(x = object@x, y = object@y - tau0 * z_num)
  ys0_mod <- do.call("ensemble_dnnet", c(list(object = ys0_obj), en_dnn_ctrl))
  ys0_hat <- as.numeric(predict(ys0_mod, object@x))
  
  semi_dat_r <- importDnnet(
    x = object@x,
    y = (object@y - tau0 * z_num - ys0_hat) / (z_num - e_hat),
    w = w
  )
